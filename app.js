...
function productCardHtml(product) {
  ...
  // Estoque baixo/esgotado continua sinalizado; estoque normal não precisa
  // de badge (reduz uma caixa colorida por card quando não há nada de
  // urgente a comunicar ao cliente).
  const lowStock = !outOfStock && available <= storeConfig.minStock;
  const stockBadge = (outOfStock || lowStock)
    ? `<span class="stock-badge ${outOfStock ? "stock-out" : "stock-low"}">${outOfStock ? "Esgotado" : `Estoque: ${available}`}</span>`
    : "";
  return `<article class="product-card" data-category="${escapeHtml(product.category || "")}" data-best-seller="${isBestSeller(product)}" data-out-of-stock="${outOfStock}">${media}<div class="product-card-body"><p class="eyebrow">${escapeHtml(product.category)}</p><h3>${name}</h3>${socialProofHtml(product)}<strong class="product-price">${currency.format(product.price)}</strong>${stockBadge}<button class="btn btn-full product-add-btn" type="button" data-add-to-cart="${escapeHtml(product.id)}" ${disabled}>${addLabel}</button><div class="product-card-secondary"><button class="product-detail-link" type="button" aria-expanded="${openProductDrawers.has(product.id)}" aria-controls="${descId}" data-toggle-desc="${escapeHtml(product.id)}">Ver detalhes</button><button class="product-whatsapp-icon" type="button" data-buy-whatsapp="${escapeHtml(product.id)}" aria-label="Perguntar sobre ${name} no WhatsApp">💬</button></div><div class="product-desc-drawer${openProductDrawers.has(product.id) ? " is-open" : ""}" id="${descId}"${openProductDrawers.has(product.id) ? "" : ' aria-hidden="true"'}><div class="product-desc-drawer-inner"><p>${escapeHtml(product.description)}</p></div></div></div></article>`;
}
