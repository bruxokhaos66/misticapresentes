...
function setupConfig() {
  ...
  $$('[data-whatsapp-link]').forEach(link => { link.href = buildWhatsappUrl("Olá, vim pelo site da Mística Presentes e gostaria de atendimento."); });
  const warnings = [];
  ...
  if (warnings.length) { publishWarning.hidden = false; publishWarning.innerHTML = `<strong>Atenção:</strong> ${warnings.join(" ")}`; }
}
...
function setupFloatingWhatsapp() {
  ...
  link.target = "_blank";
  link.rel = "noopener";
  link.setAttribute("aria-label", "Chamar Mística Presentes no WhatsApp");
...
function renderProducts() {
  productGrid.innerHTML = products.map(product => {
    ...
    const media = product.imageUrl ? `<img class="product-photo" src="${product.imageUrl}" alt="${product.name}" loading="lazy">` : `<div class="product-image" aria-hidden="true">${product.icon}</div>`;
    return `<article class="product-card">${media}<div><p class="eyebrow">${product.category}</p><h3>${product.name}</h3><p>${product.description}</p></div><strong class="product-price">${currency.format(product.price)}</strong><span class="stock-badge ${available <= storeConfig.minStock ? "stock-low" : ""}">Estoque: ${available}</span><div class="qty-row"><input id="qty-${safeId(product.id)}" type="number" min="1" max="${available}" step="1" value="1" aria-label="Quantidade de ${product.name}" ${disabled} /><button class="btn" type="button" onclick="addToCart('${product.id}')" ${disabled}>Adicionar</button></div><button class="btn btn-ghost btn-full" type="button" onclick="buyProductWhatsapp('${product.id}')">Comprar pelo WhatsApp</button></article>`;
  }).join("");
}
...
function renderCart() { if (!cart.length) cartList.innerHTML = `<div class="cart-item"><span>Nenhum produto adicionado ainda.</span></div>`; else cartList.innerHTML = cart.map(item => `<div class="cart-item"><div><strong>${item.name}</strong><span>${item.qty}x ${currency.format(item.price)} = ${currency.format(item.price * item.qty)}</span></div><button class="cart-remove" type="button" onclick="removeFromCart('${item.id}')">Remover</button></div>`).join(""); cartTotal.textContent = currency.format(getTotal()); }
function getTotal() { return cart.reduce((sum, item) => sum + item.price * item.qty, 0); }

function renderClients() { if (!clients.length) { clientList.innerHTML = `<div class="client-item"><span>Nenhum cliente cadastrado ainda.</span></div>`; return; } clientList.replaceChildren(); clients.slice(0, 5).forEach(client => { const item = document.createElement("div"); item.className = "client-item"; const name = document.createElement("strong"); name.textContent = client.name; const cpf = document.createElement("span"); cpf.textContent = `CPF: ${client.cpf}`; const whatsapp = document.createElement("span"); whatsapp.textContent = `WhatsApp: ${client.whatsapp}`; const address = document.createElement("span"); address.textContent = client.address; item.append(name, cpf, document.createElement("br"), whatsapp, document.createElement("br"), address); clientList.appendChild(item); }); }
function renderHistory() { if (!sales.length) { salesHistory.innerHTML = `<div class="history-item"><span>Nenhuma venda registrada ainda.</span></div>`; return; } salesHistory.innerHTML = sales.slice(0, 10).map(sale => `<div class="history-item"><strong>${currency.format(sale.total)} • ${new Date(sale.date).toLocaleString("pt-BR")}</strong><span>${sale.items.map(item => `${item.qty}x ${item.name}`).join(" | ")}</span><span>Status: ${sale.status}</span></div>`).join(""); }
function renderStock() { stockList.innerHTML = products.map(product => { const current = getStock(product.id); const low = current <= storeConfig.minStock; return `<div class="stock-item"><div><strong>${product.name}</strong><span>${product.category}</span></div><span class="stock-badge ${low ? "stock-low" : ""}">${current} un.</span></div>`; }).join(""); renderAdminDashboard(); }
...
function clearQrCanvas() { const ctx = pixCanvas.getContext("2d"); ctx.clearRect(0, 0, pixCanvas.width, pixCanvas.height); ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, pixCanvas.width, pixCanvas.height); }
...
function sendSaleWhatsapp() { if (!cart.length) return setStatus("Adicione produtos ao carrinho para enviar o resumo pelo WhatsApp."); window.open(buildWhatsappUrl(buildSaleSummary()), "_blank", "noopener"); }
function buyProductWhatsapp(productId) { const product = products.find(item => item.id === productId); if (!product) return; const message = `Olá, tenho interesse neste produto da ${storeConfig.name}:\n\n${product.name}\nValor: ${currency.format(product.price)}\nCategoria: ${product.category}\n\nGostaria de saber disponibilidade e opções parecidas.`; window.open(buildWhatsappUrl(message), "_blank", "noopener"); }
...
function unlockAdmin(password) { if (password !== storeConfig.adminPassword) { adminLoginStatus.hidden = false; adminLoginStatus.textContent = "Senha incorreta."; return; } sessionStorage.setItem("misticaAdminUnlocked", "true"); adminLoginPanel.hidden = true; adminContent.hidden = false; renderAdminDashboard(); renderSuppliers(); }
...
function answerIsis(prompt) { const p = prompt.toLowerCase(); if (p.includes("venda") || p.includes("fatur")) { const today = new Date(); const start = new Date(today.getFullYear(), today.getMonth(), today.getDate()); const todaySales = sales.filter(s => new Date(s.date) >= start); const total = todaySales.reduce((sum, s) => sum + s.total, 0); return `Hoje foram registradas ${todaySales.length} venda(s), totalizando ${currency.format(total)}.`; } if (p.includes("estoque")) { const low = products.filter(product => getStock(product.id) <= storeConfig.minStock); return low.length ? `Produtos com estoque baixo: ${low.map(product => `${product.name} (${getStock(product.id)} un.)`).join(", ")}.` : "Nenhum produto está abaixo do estoque mínimo."; } if (p.includes("fornecedor")) return suppliers.length ? `Fornecedores cadastrados: ${suppliers.map(s => `${s.name} (${s.category})`).join(", ")}.` : "Nenhum fornecedor cadastrado ainda."; if (p.includes("pesquisar") || p.includes("busca") || p.includes("atacado")) { const query = encodeURIComponent(prompt.replace(/pesquisar|busca|buscar/gi, "").trim() || "produtos místicos atacado"); window.open(`https://www.google.com/search?q=${query}`, "_blank", "noopener"); return "Abri uma pesquisa no navegador. Para pesquisar dentro do sistema com IA real, a próxima etapa é integrar uma API de busca/IA."; } return "Posso ajudar com: vendas de hoje, faturamento, estoque baixo, fornecedores e pesquisa de produtos. A versão atual é local; a integração com IA online pode ser adicionada no backend."; }
function handleIsisSubmit(event) { event.preventDefault(); const message = isisInput.value.trim(); if (!message) return; appendIsis("user", message); appendIsis("bot", answerIsis(message)); isisForm.reset(); }

clientForm.addEventListener("submit", event => { event.preventDefault(); const client = { name: $("#clientName").value.trim(), cpf: $("#clientCpf").value.trim(), address: $("#clientAddress").value.trim(), whatsapp: $("#clientWhatsapp").value.trim(), createdAt: new Date().toISOString() }; if (!isValidCpf(client.cpf)) { clientSaved.hidden = false; clientSaved.textContent = "CPF inválido. Confira os números digitados."; return; } if (!isValidWhatsapp(client.whatsapp)) { clientSaved.hidden = false; clientSaved.textContent = "WhatsApp inválido. Use DDD + número."; return; } clients.unshift(client); clients = clients.slice(0, 20); saveState(); renderClients(); clientSaved.hidden = false; clientSaved.textContent = `Cliente salvo: ${client.name} • ${client.whatsapp}`; clientForm.reset(); });
...
$$('[data-isis-command]').forEach(button => button.addEventListener("click", () => { const command = button.getAttribute("data-isis-command"); appendIsis("user", command); appendIsis("bot", answerIsis(command)); }));

setupConfig();
setupFloatingWhatsapp();
if (sessionStorage.getItem("misticaAdminUnlocked") === "true") { adminLoginPanel.hidden = true; adminContent.hidden = false; }
renderAll();
clearQrCanvas();
