...
function esc(value) { return (window.MisticaXSS || {}).html ? MisticaXSS.html(value) : text(value).replace(/[&<>"']/g, function(ch) { return ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[ch]; }); }
function safeUrl(value) { return (window.MisticaXSS || {}).url ? MisticaXSS.url(value) : text(value).trim(); }
...
function socialProofHtml(product) {
  ...
  return `<div class="product-rating" aria-label="Nota média ${esc(media.toFixed(1))} de 5, ${total} avaliaç${total === 1 ? "ão" : "ões"}"><span class="product-rating-stars" aria-hidden="true">${estrelas}</span><span class="product-rating-count">${esc(media.toFixed(1).replace(".", ","))} (${total})</span></div>`;
}
function productCardHtml(product) { const available = getStock(product.id); const disabled = available <= 0 ? "disabled" : ""; const media = product.imageUrl ? `<img class="product-photo" src="${esc(product.imageUrl)}" alt="${esc(product.name)}" loading="lazy">` : `<div class="product-image" aria-hidden="true">${esc(product.icon)}</div>`; const bestSeller = isBestSeller(product) ? `<span class="product-badge-best">${esc(productBadgeText(product))}</span>` : ""; return `<article class="product-card" data-category="${esc(product.category || "")}" data-best-seller="${isBestSeller(product)}">${bestSeller}${media}<div><p class="eyebrow">${esc(product.category)}</p><h3>${esc(product.name)}</h3>${socialProofHtml(product)}<p>${esc(product.description)}</p></div><strong class="product-price">${currency.format(product.price)}</strong><span class="stock-badge ${available <= storeConfig.minStock ? "stock-low" : ""}">Estoque: ${available}</span><div class="qty-row"><input id="qty-${safeId(product.id)}" type="number" min="1" max="${available}" step="1" value="1" aria-label="Quantidade de ${esc(product.name)}" ${disabled} /><button class="btn" type="button" data-action-add="${esc(product.id)}" ${disabled}>Adicionar</button></div><button class="btn btn-ghost btn-full" type="button" data-action-whatsapp="${esc(product.id)}">Comprar pelo WhatsApp</button></article>`; }
...
function renderCart() { updateCartCount(); renderCrossSell(); updateCheckoutEncomendaBox(); if (!cartList || !cartTotal) return; if (!cart.length) cartList.innerHTML = `<div class="cart-item"><span>Nenhum produto adicionado ainda.</span></div>`; else cartList.innerHTML = cart.map(item => `<div class="cart-item"><div><strong>${esc(item.name)}</strong><span>${item.qty}x ${currency.format(item.price)} = ${currency.format(item.price * item.qty)}</span>${cartItemIsEncomenda(item) ? `<span class="cart-encomenda-tag">${esc((window.misticaEncomenda && window.misticaEncomenda.BADGE) || "Sob encomenda")}</span>` : ""}</div><button class="cart-remove" type="button" data-action-remove="${esc(item.id)}">Remover</button></div>`).join(""); cartTotal.textContent = currency.format(getTotal()); }
...
function renderCrossSell() {
  ...
  box.innerHTML = `<p class="eyebrow">Combine seu ritual</p><h3>Quem levou esses, também gostou de</h3><div class="cross-sell-grid">${sugestoes.map(product => `<div class="cross-sell-card"><span aria-hidden="true">${esc(product.icon || "✨")}</span><strong>${esc(product.name)}</strong><small>${currency.format(product.price)}</small><button class="btn btn-ghost btn-full" type="button" data-action-add="${esc(product.id)}">Adicionar</button></div>`).join("")}</div>`;
}
...
function renderHistory() { if (!salesHistory) return; if (!sales.length) { salesHistory.innerHTML = `<div class="history-item"><span>Nenhuma venda registrada ainda.</span></div>`; return; } salesHistory.innerHTML = sales.slice(0, 10).map(sale => `<div class="history-item"><strong>${currency.format(sale.total)} • ${new Date(sale.date).toLocaleString("pt-BR")}</strong><span>${sale.items.map(item => `${item.qty}x ${esc(item.name)}`).join(" | ")}</span><span>Status: ${esc(sale.status)}</span></div>`).join(""); }
function renderStock() { if (!stockList) return renderAdminDashboard(); stockList.innerHTML = products.map(product => { const current = getStock(product.id); const low = current <= storeConfig.minStock; return `<div class="stock-item"><div><strong>${esc(product.name)}</strong><span>${esc(product.category)}</span></div><span class="stock-badge ${low ? "stock-low" : ""}">${current} un.</span></div>`; }).join(""); renderAdminDashboard(); }
...
function printReceipt(sale = sales[0]) { if (!sale) return alert("Nenhuma venda para imprimir."); const items = sale.items.map(item => `<tr><td>${item.qty}x ${esc(item.name)}</td><td>${currency.format(item.price * item.qty)}</td></tr>`).join(""); const win = window.open("", "_blank", "width=420,height=620"); win.document.write(`<html><head><title>Cupom ${esc(sale.id)}</title><style>body{font-family:Arial,sans-serif;padding:18px;color:#111}h1{font-size:20px}table{width:100%;border-collapse:collapse}td{border-bottom:1px dashed #999;padding:8px 0}.total{font-size:20px;font-weight:bold;text-align:right}.small{font-size:12px;color:#555}</style></head><body><h1>${esc(storeConfig.name)}</h1><p class="small">Cupom: ${esc(sale.id)}<br>${new Date(sale.date).toLocaleString("pt-BR")}</p><table>${items}</table><p class="total">Total: ${currency.format(sale.total)}</p><p>Status: ${esc(sale.status)}</p><p class="small">Obrigado pela preferência.</p></body></html>`); win.document.close(); win.focus(); win.print(); }
...
function renderSuppliers() { if (!supplierList) return; if (!suppliers.length) { supplierList.innerHTML = `<div class="history-item"><span>Nenhum fornecedor cadastrado ainda.</span></div>`; return; } supplierList.innerHTML = suppliers.map(supplier => `<div class="history-item"><strong>${esc(supplier.name)}</strong><span>${esc(supplier.category)}</span><span>WhatsApp: ${esc(supplier.whatsapp || "não informado")}</span><span>${esc(supplier.notes || "Sem observação")}</span></div>`).join(""); }
...
// Delegação de eventos para ações de produtos. Substitui os handlers inline
// (onclick="addToCart(...)") que foram removidos por razões de segurança.
// Escuta em todo o documento para cobrir cards, carrinho e cross-sell.
document.addEventListener("click", function (event) {
  const addBtn = event.target.closest("[data-action-add]");
  if (addBtn) { addToCart(addBtn.dataset.actionAdd); return; }
  const whatsappBtn = event.target.closest("[data-action-whatsapp]");
  if (whatsappBtn) { buyProductWhatsapp(whatsappBtn.dataset.actionWhatsapp); return; }
  const removeBtn = event.target.closest("[data-action-remove]");
  if (removeBtn) { removeFromCart(removeBtn.dataset.actionRemove); return; }
});
