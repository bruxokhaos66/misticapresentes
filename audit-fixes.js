(() => {
  ...
  const css = `
    ...
    .isis-panel-image .isis-human-img{display:block!important;width:min(100%,460px)!important;max-height:660px!important;object-fit:contain!important;margin:0 auto 18px!important;filter:drop-shadow(0 28px 55px rgba(0,0,0,.45))!important;opacity:1!important;visibility:visible!important;}
    ...
    @media(max-width:560px){.isis-panel-image .isis-human-img{max-height:500px!important;}.brand-logo-modern{width:46px!important;height:46px!important;}}
  ...
  const forceIsisImage = () => {
    ...
    if (!panel) return;
    const version = "20260706-isis-publicitaria";
    const sources = [`assets/isis-humana-xamanica-02-publicitaria.webp?v=${version}`, `./assets/isis-humana-xamanica-02-publicitaria.webp?v=${version}`, `/assets/isis-humana-xamanica-02-publicitaria.webp?v=${version}`];
    let attempt = 0;
    const render = src => {
      panel.classList.remove("asset-failed");
      panel.innerHTML = `<img class="isis-human-img isis-human-publicitaria" src="${src}" alt="Isis da Mística Presentes" width="720" height="900" loading="eager" decoding="async"><p>Isis, presença misteriosa e xamânica para guiar escolhas e atendimento da loja.</p>`;
