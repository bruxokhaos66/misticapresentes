(() => {
  ...
  const css = `
    ...
    select:focus-visible{
      outline:3px solid #f0c56a!important;
      outline-offset:4px!important;
      box-shadow:0 0 0 6px rgba(240,197,106,.18)!important;
    }
    .highlight-strip{
  ...
  const improveButtons = () => {
    ...
    if (menuButton && navLinks) {
      ...
      menuButton.addEventListener("click", () => {
        setTimeout(() => menuButton.setAttribute("aria-expanded", String(navLinks.classList.contains("is-open"))), 0);
      });
    }
    ...
    const observer = new MutationObserver(records => {
      records.forEach(record => {
        record.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) improveImages(node);
        });
      });
    });
