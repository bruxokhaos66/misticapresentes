...
def init_db():
    ...
    # Compatibilidade com bancos antigos: garante colunas usadas nas abas Clientes/Marketing
    ...
    # Migração de bancos antigos que usavam a coluna telephone.
    try:
        query_db("ALTER TABLE clientes ADD COLUMN telephone TEXT", commit=True)
    except Exception:
        pass
    try:
    ...
    # Isis inteligente: logs, pesquisas, encomendas e memoria categorizada
    ...
    for col, typ in [("ativo", "INTEGER DEFAULT 1")]:
        try:
            ...
            pass
    for col, typ in [
        ("dinheiro_sistema", "REAL DEFAULT 0.0"), ("pix_sistema", "REAL DEFAULT 0.0"),
        ("debito_sistema", "REAL DEFAULT 0.0"), ("credito_sistema", "REAL DEFAULT 0.0"),
        ("dinheiro_informado", "REAL DEFAULT 0.0"), ("pix_informado", "REAL DEFAULT 0.0"),
        ("debito_informado", "REAL DEFAULT 0.0"), ("credito_informado", "REAL DEFAULT 0.0"),
        ("diferenca_caixa", "REAL DEFAULT 0.0")
    ]:
        try:
    ...
    for col, typ in [("custo", "REAL DEFAULT 0.0"), ("lucro", "REAL DEFAULT 0.0"), ("estoque_minimo", "INTEGER DEFAULT 0")]:
        try:
            ...
            pass
    try:
        query_db("ALTER TABLE vendas ADD COLUMN status TEXT DEFAULT 'Concluído'", commit=True)
    except Exception:
        pass
    try:
        query_db("ALTER TABLE vendas ADD COLUMN data_iso TEXT", commit=True)
    except Exception:
        pass
    try:
    ...
    try:
        ...
        pass
    try:
        query_db("ALTER TABLE contas_a_pagar ADD COLUMN categoria TEXT DEFAULT 'Outros'", commit=True)
    except Exception:
        pass
    try:
        query_db("ALTER TABLE fluxo_caixa ADD COLUMN caixa_id INTEGER", commit=True)
    except Exception:
        pass

    # Correções profissionais de compatibilidade com bancos antigos
    for tabela, colunas in {
        "categorias": [("ativo", "INTEGER DEFAULT 1"), ("excluido_em", "TEXT")],
        "clientes": [("ativo", "INTEGER DEFAULT 1"), ("excluido_em", "TEXT")],
        "usuarios": [("cpf", "TEXT"), ("endereco", "TEXT"), ("telefone", "TEXT"), ("perfil", "TEXT"), ("ativo", "INTEGER DEFAULT 1"), ("excluido_em", "TEXT")],
    ...
    # Índices para melhorar desempenho em buscas, relatórios e vendas
    for sql_idx in [
    ...
    if not admin_res:
        ...
        query_db("INSERT INTO usuarios (nome, login, senha_hash, perfil, ativo) VALUES (?,?,?,?,?)", ("Administrador", "admin", hash_password_pbkdf2(senha_temp), "adm", 1), commit=True)
        try:
        ...
        senha_temp = "Mistica@" + secrets.token_hex(4)
        query_db("UPDATE usuarios SET senha_hash=?, ativo=1 WHERE login='admin'", (hash_password_pbkdf2(senha_temp),), commit=True)
        try:
            with open(os.path.join(DOCS_PATH, "mistica_senha_admin_inicial.txt"), "w", encoding="utf-8") as f:
