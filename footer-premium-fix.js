(() => {
  ...
  function installFooterStyle() {
    ...
    style.textContent = `
      ...
      .footer::before { content: ""; position: absolute; inset: 0; pointer-events: none; background: linear-gradient(90deg, transparent, rgba(240,197,106,.10), transparent), radial-gradient(circle at 50% 0, rgba(240,197,106,.12), transparent 34rem); opacity: .82; }
      ...
      .footer-grid { position: relative; z-index: 1; gap: clamp(18px, 3vw, 32px); }
      .footer-grid > div { min-height: 100%; border: 1px solid rgba(240,197,106,.18); border-radius: 28px; padding: clamp(20px, 3vw, 30px); background: linear-gradient(145deg, rgba(255,248,230,.07), rgba(83,107,55,.055)), rgba(3,3,5,.24); box-shadow: 0 22px 70px rgba(0,0,0,.22); backdrop-filter: blur(10px); }
      .footer-grid > div:first-child { border-color: rgba(240,197,106,.30); background: radial-gradient(circle at 18% 10%, rgba(240,197,106,.16), transparent 34%), linear-gradient(145deg, rgba(255,248,230,.09), rgba(54,32,68,.12)); }
      .footer-mark { margin-bottom: 16px; box-shadow: 0 0 0 7px rgba(240,197,106,.055), 0 18px 42px rgba(0,0,0,.32); }
      .footer strong { margin-bottom: 9px; font-size: clamp(1.18rem, 2vw, 1.56rem); letter-spacing: .08em; text-transform: uppercase; }
      .footer h3 { margin-bottom: 12px; font-family: Cinzel, Georgia, serif; letter-spacing: .08em; text-transform: uppercase; }
      .footer p { margin-bottom: 9px; color: #e5d8bf; font-weight: 650; }
      .footer a, .footer p:last-child { color: #b8c977; }
      .footer-premium-signature { position: relative; z-index: 1; width: min(1180px, calc(100% - 32px)); margin: 22px auto 0; display: flex; flex-wrap: wrap; gap: 10px 16px; align-items: center; justify-content: space-between; border: 1px solid rgba(240,197,106,.16); border-radius: 999px; padding: 12px 16px; background: rgba(255,248,230,.045); color: #d8cbb6; font-size: .82rem; font-weight: 800; }
      .footer-premium-signature span:first-child { color: #f0c56a; letter-spacing: .08em; text-transform: uppercase; }
      @media (max-width: 680px) { .footer { padding-bottom: 86px; } .footer-grid > div { border-radius: 24px; } .footer-premium-signature { border-radius: 24px; align-items: flex-start; justify-content: flex-start; } }
    `;
