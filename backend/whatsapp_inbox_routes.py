...
exigem sessão administrativa real (backend.panel_sessions.exigir_perfil,
que também valida Origin/Referer em métodos mutáveis -- proteção CSRF de
defesa em profundidade além do cookie SameSite=Lax). Nunca aceita chave de
API estática aqui (diferente de outras integrações servidor-a-servidor):
esta é uma ferramenta de atendimento operado por humanos.
...
@router.get("/status")
def status_central_atendimento(response: Response, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _sem_cache(response)
...
def rota_listar_conversas(
    ...
    sessao: dict = Depends(exigir_perfil(perfil_minimo="adm")),
):
    _sem_cache(response)
    _exigir_habilitado()
...
def rota_obter_conversa(conversation_id: int, response: Response, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _sem_cache(response)
    _exigir_habilitado()
    with conectar() as conn:
...
def rota_listar_mensagens(
    ...
    _exigir_habilitado()
    with conectar() as conn:
        _obter_conversa_ou_404(conn, conversation_id)
...
def rota_marcar_lida(conversation_id: int, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _exigir_habilitado()
    usuario = str(sessao.get("nome") or sessao.get("login") or "Admin")
    with conectar() as conn:
        _obter_conversa_ou_404(conn, conversation_id)
...
def rota_atualizar_conversa(conversation_id: int, body: AtualizarConversaBody, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _exigir_habilitado()
    usuario = str(sessao.get("nome") or sessao.get("login") or "Admin")
...
def rota_vincular_cliente(conversation_id: int, body: VincularClienteBody, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _exigir_habilitado()
    usuario = str(sessao.get("nome") or sessao.get("login") or "Admin")
    with conectar() as conn:
        _obter_conversa_ou_404(conn, conversation_id)
...
def rota_vincular_pedido(conversation_id: int, body: VincularPedidoBody, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _exigir_habilitado()
    usuario = str(sessao.get("nome") or sessao.get("login") or "Admin")
    with conectar() as conn:
        _obter_conversa_ou_404(conn, conversation_id)
...
def rota_obter_midia(message_id: int, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
    _exigir_habilitado()
    with conectar() as conn:
...
def rota_listar_templates(response: Response, sessao: dict = Depends(exigir_perfil(perfil_minimo="adm"))):
