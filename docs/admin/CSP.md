...
| `www.mercadopago.com` | script-src | Script oficial de Device ID (antifraude): `https://www.mercadopago.com/v2/security.js?view=checkout`, carregado DINAMICAMENTE por `v2-mercadopago-checkout.js::carregarScriptDeviceId()` -- só quando o cliente abre "Cartão de crédito" (mesmo padrão de `carregarSdk()` para o SDK principal), nunca em toda visita à página. Cria a variável global `window.MP_DEVICE_SESSION_ID`, lida por `obterDeviceId()` no instante do submit do cartão e encaminhada pelo backend ao Mercado Pago no header `X-meli-session-id` (nunca no corpo JSON, nunca persistida, nunca logada). Host exato, sem curinga -- mesmo padrão de `sdk.mercadopago.com`. Carregar isso incondicionalmente em `index.html` reduziu o score de `best-practices` do Lighthouse CI (ver seção abaixo) -- por isso o carregamento é sempre sob demanda. |
