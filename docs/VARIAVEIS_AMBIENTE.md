...
| `MISTICA_ADMIN_PASSWORD` | Senha do usuário administrativo criado/redefinido automaticamente no startup (login definido por `MISTICA_ADMIN_LOGIN`). | Sim | Produção | `senha-forte-fake` | Nenhum admin automático é criado/redefinido; o usuário pré-existente continua com a senha antiga. | **Sim** |
| `MISTICA_ADMIN_LOGIN` | Login do usuário administrativo criado/redefinido automaticamente no startup (ver `MISTICA_ADMIN_PASSWORD`). Só tem efeito se `MISTICA_ADMIN_PASSWORD` também estiver definida. | Não (padrão `admin`) | Produção | `bruxo-mor` | Usa o padrão `admin`, igual ao comportamento anterior a esta variável — instalações existentes não são afetadas. | Não |
...
| `MISTICA_REST_ESTORNO_ENABLED` | Libera a rota `POST /api/vendas/{id}/estornar`, que devolve estoque mas **não** reverte o lançamento financeiro em `fluxo_caixa` (a venda não é associada ao `caixa_id` original — issue #335). Independente de `APP_ENV`; não pode ser ligada por hostname nem por query string. | Não (padrão desligada, em qualquer ambiente) | Produção | `false` | Rota responde `404` sem consultar/alterar a venda, sem tocar estoque, sem escrever `fluxo_caixa`, sem gravar auditoria de estorno. **Mantenha `false` em produção até a correção estrutural da issue #335.** | Não (é `true`/`false`, não um segredo) |
