...
| Variável | Finalidade | Obrigatória? | Ambiente | Exemplo | Comportamento se ausente | Sensível? |
|---|---|---|---|---|---|---|
| `APP_ENV` | Ambiente de execução; controla a lista de origens CORS permitidas. | Sim | Produção | `production` | Cai em `development`, liberando origens de CORS mais permissivas (pensadas para localhost) — inadequado em produção. | Não |
