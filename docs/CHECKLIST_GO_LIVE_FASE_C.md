...
| Estorno via API não reflete `fluxo_caixa` (issue #335) | APROVADO COM RISCO | Correção estrutural (associação venda→`caixa_id`) ainda pendente, exige migração de schema. Mitigação temporária aplicada (PR `fix/restringe-estorno-rest-producao`): a rota `POST /api/vendas/{id}/estornar` fica atrás da feature flag `MISTICA_REST_ESTORNO_ENABLED`, desligada por padrão em qualquer ambiente — com a flag desligada (o estado exigido em produção), a rota responde `404` sem tocar venda/estoque/`fluxo_caixa`/auditoria. Risco residual só existe se alguém ligar a flag manualmente sem a correção estrutural. |
